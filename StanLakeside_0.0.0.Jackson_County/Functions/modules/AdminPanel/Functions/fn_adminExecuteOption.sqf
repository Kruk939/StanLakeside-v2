...
if(_statementsent == 18) then {
	...
	if(!adminESP) exitwith { adminESP = true; RPF_ID_PlayerTags = ["RPF_PlayerTags","onEachFrame","client_fnc_adminESP"] call BIS_fnc_addStackedEventHandler; [player,objNull,17,format ["%1 włączył adminESP",name player],""] remoteExec ["server_fnc_adminLog", 2]; };
	if(adminESP) exitwith { adminESP = Nil; ["RPF_PlayerTags", "onEachFrame"] call BIS_fnc_removeStackedEventHandler; [player,objNull,18,format ["%1 wyłączył adminESP",name player],""] remoteExec ["server_fnc_adminLog", 2];};
};
