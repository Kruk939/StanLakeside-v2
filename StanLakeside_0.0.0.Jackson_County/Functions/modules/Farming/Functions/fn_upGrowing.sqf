...
if(_checkFinish) exitwith { ["Nie tak szybko!", false] spawn ClientModules_Notification_fnc_doMsg; };
...
if(_requiredOutput == _usedAction) then { 
	...
	["To nie był dobry wybór..!", false] spawn ClientModules_Notification_fnc_doMsg;
	_randomValue = random 30;
	if(_randomValue < 5) exitwith { 
		["No cóż... schrzaniłeś!", false] spawn ClientModules_Notification_fnc_doMsg;
