...
private["_status","_lostprice","_typebuy","_index","_quantity","_item","_itemName","_cost","_type","_totalprice","_error","_rarefish","_backpackitems","_rarefish","_rarefish1","_rarefish2","_rarefish3","_rarefish4"];
...
if(_typebuy == "standard") then {
	if(!([1, _totalprice] call Client_fnc_checkMoney)) exitwith { hint "Za malo pieniędzy."; _error = true; };
};
...
if(_typebuy == "standard") then {
	...
	if(_lostprice > 0) then {
		if (Mayor_inited) then {[_lostprice] spawn ClientModules_Mayor_fnc_payTaxes; };
	};
};
