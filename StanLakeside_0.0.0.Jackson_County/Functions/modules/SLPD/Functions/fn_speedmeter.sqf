	...
	if (player getVariable ['cop', 0] > 0 || player getVariable ['ems', 0] > 0) then {} else {
		...
		if(vehicle player != player && driver (vehicle player) isEqualTo player && ((_vehicle isKindOf "Car") || (_vehicle isKindOf "Motorcycle") || (_vehicle isKindOf "Bicycle") || (_vehicle isKindOf "Motorbike"))) then {
			...
			if(_handle && !speedMeter_last_ticket) then {
				...
				["vehicle", _data] remoteExec ["ServerModules_fnc_slpdCaseAdd",2];
				...
				//[_message] spawn {
					...
					//uiSleep 60;
					//["fotoradar",player,_message,"Wydział ruchu drogowego","[Mandat] Wydział ruchu drogowego"] remoteExec ["ServerModules_fnc_sendMail",2];
