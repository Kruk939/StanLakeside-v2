...
while{ClientArrested} do {
	...
	if(_time < 0) exitwith {
		...
		["ended", [getPlayerUID player]] remoteExec ["ServerModules_SLPD_fnc_slpdPrisonUpdate",2];
	};
	if(_update == 5) then {
		...
		if(_time_left > 0) then {
			...
			_data set [0, _first];
			["time", [_time, _id]] remoteExec ["ServerModules_SLPD_fnc_slpdPrisonUpdate",2];
		} else {
			_data deleteAt 0;
			_id = _first select 0;
			_first = _data select 0;
			...
			["finish", [_id]] remoteExec ["ServerModules_SLPD_fnc_slpdPrisonUpdate",2];
		};
		_update = 0;
	};
};
